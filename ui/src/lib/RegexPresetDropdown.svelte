<script lang="ts">
  import { REGEX_PRESETS, type RegexFieldType } from './regexPresets';

  let { fieldType, onSelect }: { fieldType: RegexFieldType; onSelect: (pattern: string) => void } =
    $props();

  let open = $state(false);

  let presets = $derived(REGEX_PRESETS[fieldType] ?? []);
